...
rDAG <- function(p, n, maxInDegree = 4, dist = "unif", lowScale = 1, highScale = 1, lowEdge = .3, highEdge = 1){
  ...
  for(i in 2:p){
    ...
    if(maxInDegree > 1){
      ...
      numParents <- sample.int(min(maxInDegree - 1, i - 1), size = 1)
      
      # ensure that i-1 is parent so that there is a unique ordering
      parents <- c(sample(1:(i-1), size = numParents))
      # set edge weights
      B[i, parents] <- runif(length(parents), lowEdge, highEdge) * sample(c(-1, 1), size = length(parents), replace = T)
    }
